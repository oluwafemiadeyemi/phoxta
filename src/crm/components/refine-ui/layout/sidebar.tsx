...
import { useMenu, useLink, type TreeMenuItem, useRefineOptions } from "@refinedev/core";
import {
...
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@crm/components/ui/collapsible";
...
function SidebarItemDropdown({ item, selectedKey }: MenuItemProps) {
  ...
  const handleNavigate = () => {
    if (isMobile) {
      // Smooth transition: delay closing to allow navigation to start first
      setTimeout(() => {
        setOpenMobile(false);
      }, 100);
    }
...
function SidebarButton({
  ...
  const handleClick = (event: React.MouseEvent<HTMLElement>) => {
    ...
    if (isMobile && asLink) {
      // Smooth transition: delay closing to allow navigation to start first
      // Only close for leaf links, not collapsible parent items
      setTimeout(() => {
        setOpenMobile(false);
      }, 100);
    }
